...
export function TimelineBlock({ block }: TimelineBlockProps) {
  ...
  return (
    <section className="container py-12">
      ...
      {description && <p className="mb-8 text-muted">{description}</p>}
      <div className="space-y-6 border-l border-default pl-6">
        {events.map((event, index) => (
          <div key={index} className="relative">
            <span className="absolute -left-[30px] top-2 h-3 w-3 rounded-full bg-gray-300" />
            {event.date && <div className="text-xs uppercase text-muted">{event.date}</div>}

...
| `custom-items` collection is fully public (`access.read: () => true`) and ignores `status`, exposing drafts/archived items via REST/GraphQL | Fixed | 2026-01-15 | 2026-01-15 | Read access now limits guests to `status=published`; publish sets `publishedAt` |
| New collections (`custom-items`, `content-types`) are not wired into SEO/search/revalidation/tagging | Fixed (custom-items) | 2026-01-15 | 2026-01-15 | `custom-items` added to SEO/search plugins and revalidation hook; content-types intentionally not indexed |

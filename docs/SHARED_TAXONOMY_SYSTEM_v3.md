...
**Grouping / Curated Content Types** (usually no taxonomy):
- Example only: “Museum Collections” (galleries/exhibitions)
- These are typically custom content types created from base templates
- Places are NOT excluded from taxonomy and may use Categories/Tags (e.g. city, town, region)
...
## Clarification: Base Templates vs Custom Content Types (IMPORTANT)

The system ships with a **small set of base content templates only**:
- Archive Item
- Product
- Person
- Place
...
All other domain-specific types (e.g. Museum Collections, Galleries, Exhibitions,
Portfolios, Casts, Actors, Venues, etc.) are **not shipped as first-class collections**.

Instead, they are created via **Admin → Content Types** as **custom content types**
that *extend one of the base templates*.

This means:
- There is no special system-level concept called a “Grouping Collection”
- Any content type that groups other items is simply a **custom content type**
- Grouping behaviour is achieved through relationships, not taxonomy

Examples:
- “Exhibition” → custom type based on **Event**
- “Gallery” or “Collection” → custom type based on **Archive Item**
- “Venue” → custom type based on **Place**
- “Cast” or “Actors” → custom type based on **Person**

This preserves a clean core system while allowing unlimited extension without
shipping niche collections by default.
...
- [ ] Update navigation to show everything correctly - make sure it is dyanmic and shows any new types dynamically
